...
Mean number of steps taken per day is `r mn` and the median is `r md`.
...
Interval `r interval.with.highest.avg.steps` has the highest average number of steps.
...
Total number of of rows with missing values is `r missing.rows`. 
...
In the clean dataset nean number of steps taken per day is `r mn.clean` and. the median is `r md.clean` which is different from the dataset with NA's. 
